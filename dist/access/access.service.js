"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AccessService = void 0;
const common_1 = require("@nestjs/common");
const api_key_service_1 = require("../core/apiKey/api-key.service");
let AccessService = class AccessService {
    constructor(apiKeyService) {
        this.apiKeyService = apiKeyService;
    }
    async createApiKey(createApiKeyDto) {
        return this.apiKeyService.createApiKey(createApiKeyDto);
    }
    async useApiKey(useApiKeyDto) {
        return this.apiKeyService.useApiKey(useApiKeyDto);
    }
    async revokeApiKey(revokeApiKeyDto) {
        return this.apiKeyService.removeApiKey(revokeApiKeyDto);
    }
    async getTokens(getTokensDto) {
        const getAllApiTokens = await this.apiKeyService.getUserApiKeysByUserId(getTokensDto.userId);
        const result = [];
        let view;
        for (const token of getAllApiTokens) {
            const getApiDetails = await this.apiKeyService.getUserDataByToken(token);
            const getLastTimeUsed = await this.apiKeyService.getLastTimeUsed(token, getTokensDto.userId);
            view = this._createCustomView(token, getApiDetails, getLastTimeUsed);
            result.push(view);
        }
        return result;
    }
    _createCustomView(token, getApiDetails, lastTimeUsed) {
        return {
            token: token.slice(-4),
            permissions: getApiDetails.permissions,
            lastTimeUsedApiKey: lastTimeUsed,
        };
    }
};
AccessService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [api_key_service_1.ApiKeyService])
], AccessService);
exports.AccessService = AccessService;
//# sourceMappingURL=access.service.js.map